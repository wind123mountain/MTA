...
def compute_hidden_span_loss(projector, s_hidden_state, t_hidden_state, All_Indices, 
    ...
    if args.use_dsa:    
        ...
        Valid_Pair = (Valid_Pair_Weights > 0).float()
        span_loss = (span_loss * Valid_Pair).sum() / Valid_Pair.sum().clamp(min=1e-5)
    ...
    if args.use_hs:
        ...
        valid = (T_Token_Weights_all > 0).float()
        token_loss = (token_loss * valid).sum() / valid.sum().clamp(min=1e-5)
...
def get_spans_offsets(texts, nlp, matcher):
    ...
    for doc in nlp.pipe(texts, disable=disabled_components, n_process=4):
        ...
        #     doc_phrases.append((sent.start_char, sent.root.idx))
        #     doc_phrases.append((sent.root.idx, sent.end_char + 1))
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if teacher_model is not None:
                ...
                fdd_loss = get_fdd_loss(args, teacher_outputs.hidden_states, outputs.hidden_states, 
                ...
                phrases_offsets, spans_offsets, words_offsets = get_spans_offsets(input_texts, nlp, matcher)
                ...
                loss = (1 - args.kd_ratio) * lm_loss + args.kd_ratio * (distil_loss + fdd_loss + span_loss)
            else:
                loss = lm_loss
            ...
            if global_step % args.log_interval == 0 and step % args.gradient_accumulation_steps == 0:
                ...
                print_rank(f"GPU allocated: {allocated} %")
